...
from common.net import DCGANGenerator as Generator

from net import Discriminator, Denoiser
...
def main():
    ...
    report_keys = ["loss_dis", "loss_gen", "loss_den", "inception_mean", "inception_std", "FID"]
    max_iter = args.max_iter

    if args.gpu >= 0:
        chainer.cuda.get_device_from_id(args.gpu).use()

    generator = Generator(z_distribution="uniform", use_bn=True)
    ...
    # Setup an optimizer
    def make_optimizer(model, alpha=0.0001, beta1=0.5, beta2=0.9):
    ...
    opt_den = make_optimizer(denoiser)
    ...
    trainer.extend(extensions.snapshot_object(
        denoiser, 'denoiser_{.updater.iteration}.npz'), trigger=(args.snapshot_interval, 'iteration'))
